...
const joinCommand: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction, client: Client) {
    ...
    const voiceChannel = interaction.member.voice.channel as VoiceBasedChannel | null;
    ...
    try {
      ...
      if (voiceChannel.joinable) {
        ...
        const embed = new EmbedBuilder()
          .setTitle('Error')
          .setDescription(`I don't have permission to join ${voiceChannel.name}.`)
          .setColor('#ff0000');
      ...
      const embed = new EmbedBuilder()
        .setTitle('Error')
        .setDescription('There was an error while trying to join the voice channel.')
        .setColor('#ff0000');
